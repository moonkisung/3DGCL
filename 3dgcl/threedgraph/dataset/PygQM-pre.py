...
class QM(InMemoryDataset):
    ...
    def process(self):
        ...
        for i, line in enumerate(tqdm(dataset)):
            ...
            ## UFF ##
            AllChem.EmbedMultipleConfs(mol, numConfs=3, maxAttempts=2000, randomSeed=2222, numThreads=0)
            try:
            ...
            if 0.0 in minpos_uff or 0.0 in minpos_mmff :
                continue
            ...
            if self.name == 'qm8':
                ...
                data = Data(pos=minpos_uff, max1pos_uff=max1pos_uff, max2pos_uff=max2pos_uff, 
                            minpos_mmff=minpos_mmff, max1pos_mmff=max1pos_mmff, max2pos_mmff=max2pos_mmff,
                            z=z, smiles=smiles,
                            E1_CC2=y[0], E2_CC2=y[1], f1_CC2=y[2], f2_CC2=y[3], E1_PBE0=y[4], E2_PBE0=y[5], f1_PBE0=y[6], f2_PBE0=y[7],
                            E1_CAM=y[12], E2_CAM=y[13], f1_CAM=y[14], f2_CAM=y[15])                          
            else :
                data = Data(pos=minpos_etkdg, max1pos_etkdg=max1pos_etkdg, max2pos_etkdg=max2pos_etkdg, z=z, y=y,smiles=smiles)
