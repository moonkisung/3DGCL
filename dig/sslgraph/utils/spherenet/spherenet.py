...
from .features import dist_emb, angle_emb, torsion_emb, xyz_to_dat
...
class update_v(torch.nn.Module):
    def __init__(self, hidden_channels, out_emb_channels, num_output_layers, act, output_init, dropout_rate):
        ...
        self.lin = nn.Linear(out_emb_channels, hidden_channels, bias=False)
        self.dropout = nn.Dropout(dropout_rate)
        self.reset_parameters()
...
class SphereNet(torch.nn.Module):
    ...
    """
    def __init__(
        self, cutoff, num_layers, hidden_channels, int_emb_size, basis_emb_size_dist,
        basis_emb_size_angle, basis_emb_size_torsion, out_emb_channels, num_spherical, num_radial, envelope_exponent,
        num_before_skip, num_after_skip, num_output_layers, dropout_rate, use_node_features,
        act=swish, output_init='GlorotOrthogonal'):
        super(SphereNet, self).__init__()
        
        self.cutoff = cutoff
        self.num_layers = num_layers
        self.hidden_channels = num_layers
    ...
    #def __init__(self, num_radial, hidden_channels, dropout_rate, use_node_features, act=swish):
        ...
        self.init_e = init(num_radial, hidden_channels, dropout_rate, use_node_features, act)
        self.init_v = update_v(hidden_channels, out_emb_channels, num_output_layers, act, output_init, self.dropout_rate)
        self.init_u = update_u()
        self.emb = emb(num_spherical, num_radial, self.cutoff, envelope_exponent)

        self.update_vs = torch.nn.ModuleList([
            update_v(hidden_channels, out_emb_channels, num_output_layers, act, output_init, self.dropout_rate) for _ in range(num_layers)])
        self.update_es = torch.nn.ModuleList([
            update_e(hidden_channels, int_emb_size, basis_emb_size_dist, basis_emb_size_angle, basis_emb_size_torsion, num_spherical, num_radial, num_before_skip, num_after_skip, dropout_rate=self.dropout_rate) for _ in range(num_layers)])
