...
class NodeAttrMask():
    ...
    '''
    def __init__(self, encoder, mode='whole', mask_ratio=0.2, mask_mean=0.5, mask_std=0.5, return_mask=False):
    ...
    def do_trans(self, data):
        node_num = len(data.z)
        #print(node_num)
        #x = data.z.detach().clone().cpu()
        x = data.z.cpu()
        #print(x)
        if self.mode == 'whole':
            mask = torch.zeros(node_num)
            #mask_num = int(node_num * self.mask_ratio) # the number of masking node
            idx_mask = np.random.choice(node_num, int(node_num * self.mask_ratio), replace=False) # idx of masking node
            x[idx_mask] = 0
        ...
        if self.return_mask:
            return Data(pos=data.pos, smiles=data.smiles, z=x, mask=mask)
        else:
            #return Data(x=x, edge_index=data.edge_index)
            return Data(pos=data.pos, smiles=data.smiles, z=x)
