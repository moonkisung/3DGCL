...
class ThreeDEvaluator_rmse:
    ...
    def eval(self, input_dict):
        ...
        if isinstance(y_true, torch.Tensor):
            return {'rmse': torch.sqrt(torch.mean(torch.square(y_pred - y_true))).cpu().item()}
        else:
            return {'rmse': float(np.sqrt(np.mean(np.square(y_pred - y_true))))}