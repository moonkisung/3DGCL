...
class run_rmse():
    ...
    def run_rmse(self, device, train_loader, val_loader, test_loader, model, loss_func, evaluation, epochs=500, batch_size=32, vt_batch_size=32, lr=0.0005, lr_decay_factor=0.5, lr_decay_step_size=50, weight_decay=0, 
        ...
        for epoch in range(1, epochs + 1):
            ...
            if log_dir != '':
                writer.add_scalar('train_rmse', train_mae, epoch)
                writer.add_scalar('valid_rmse', valid_mae, epoch)
                writer.add_scalar('test_rmse', test_mae, epoch)
            ...
            if valid_mae < best_valid:
                ...
                if save_dir != '':
                    ...
                    checkpoint = {'epoch': epoch, 'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict(), 'scheduler_state_dict': scheduler.state_dict(), 'best_valid_rmse': best_valid, 'num_params': num_params}
                    torch.save(checkpoint, os.path.join(save_dir, 'valid_checkpoint.pt'))

            scheduler.step()

        print(f'Best validation RMSE so far: {best_valid}')
        print(f'Test RMSE when got best validation result: {best_test}')
    ...
    def train(self, model, optimizer, train_loader, energy_and_force, p, loss_func, device):
        ...
        for step, batch_data in enumerate(tqdm(train_loader)):
            ...
            if energy_and_force:
                ...
                e_loss = torch.sqrt(loss_func(out, batch_data.y.unsqueeze(1)))
                f_loss = torch.sqrt(loss_func(force, batch_data.force))
                loss = e_loss + p * f_loss
            else:
                loss = torch.sqrt(loss_func(out, batch_data.y.unsqueeze(1)))
            loss.backward()
            optimizer.step()
    ...
    def val(self, model, data_loader, energy_and_force, p, evaluation, device):
        ...
        if energy_and_force:
            ...
            energy_mae = evaluation.eval(input_dict)['rmse']
            force_mae = evaluation.eval(input_dict_force)['rmse']
            print({'Energy RMSE': energy_mae, 'Force RMSE': force_mae})
            return energy_mae + p * force_mae

        return evaluation.eval(input_dict)['rmse']
