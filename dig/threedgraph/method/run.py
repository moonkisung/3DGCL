...
class run():
    ...
    def run(self, device, train_dataset, valid_dataset, test_dataset, model, loss_func, evaluation, epochs=500, batch_size=32, vt_batch_size=32, lr=0.0005, lr_decay_factor=0.5, lr_decay_step_size=50, weight_decay=0, 
        ...
        for epoch in range(1, epochs + 1):
            ...
            print('\nTraining...', flush=True)
            train_loss = self.train(model, optimizer, train_loader, energy_and_force, p, loss_func, device)

            print('\n\nEvaluating...', flush=True)
            valid_loss = self.val(model, valid_loader, energy_and_force, p, evaluation, device)

            print('\n\nTesting...', flush=True)
            test_loss = self.val(model, test_loader, energy_and_force, p, evaluation, device)
            ...
            if valid_loss < best_valid:
                ...
                if save_dir != '':
                    ...
                    checkpoint = {'epoch': epoch, 'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict(), 'scheduler_state_dict': scheduler.state_dict(), 'best_valid_loss': best_valid, 'num_params': num_params}
    ...
    def train(self, model, optimizer, train_loader, energy_and_force, p, loss_func, device):
        ...
        for step, batch_data in enumerate(tqdm(train_loader)):
            ...
            if energy_and_force:
                ...
                #loss = loss_func(out, batch_data.y.unsqueeze(1).to(dtype=torch.long))  # regression
    ...
    def val(self, model, data_loader, energy_and_force, p, evaluation, device):
        ...
        #return evaluation.eval(input_dict)['auc']
