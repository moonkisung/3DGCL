...
class emb(torch.nn.Module):
    ...
    def forward(self, dist, angle, torsion, idx_kj):
        ...
        #print('the number of angle nan', torch.sum(torch.isnan(angle)))
        dist_emb = self.dist_emb(dist)
        #print('dist_emb', dist_emb)
        #print('dist_emb.shape', dist_emb.shape)
        #print('the number of dist_emb nan', torch.sum(torch.isnan(dist_emb)))
        angle_emb = self.angle_emb(dist, angle, idx_kj)
        #print('angle_emb', angle_emb)
        #print('angle_emb.shape', angle_emb.shape)
        #print('the number of angle_emb nan', torch.sum(torch.isnan(angle_emb)))
        torsion_emb = self.torsion_emb(dist, angle, torsion, idx_kj)
        #print('torsion_emb', torsion_emb)
        #print('torsion_emb.shape', torsion_emb.shape)
        #print('the number of torsion_emb nan', torch.sum(torch.isnan(torsion_emb)))
...
class init(torch.nn.Module):
    def __init__(self, num_radial, hidden_channels, dropout_rate, use_node_features, act=swish):
...
class update_e(torch.nn.Module):
    def __init__(self, hidden_channels, int_emb_size, basis_emb_size_dist, basis_emb_size_angle, basis_emb_size_torsion, num_spherical, num_radial,
        num_before_skip, num_after_skip, dropout_rate, act=swish):
    ...
    def forward(self, x, emb, idx_kj, idx_ji):
        ...
        for layer in self.layers_before_skip:
            e1 = layer(e1)
            e1 = self.dropout(e1)
        e1 = self.act(self.lin(e1)) + x1  # nan
...
class update_v(torch.nn.Module):
    def __init__(self, hidden_channels, out_emb_channels, out_channels, num_output_layers, dropout_rate, act, output_init):
...
class update_u(torch.nn.Module):
    ...
    def forward(self, u, v, batch):
        ...
        #print('the number of v nan', torch.sum(torch.isnan(v)))
        u += scatter(v, batch, dim=0)
        #print('u', u)
        #print('u.shape', u.shape)
        #print('the number of u nan', torch.sum(torch.isnan(u)))
        return u
...
class SphereNet(torch.nn.Module):
    ...
    """
    def __init__(
        self, cutoff, num_layers, hidden_channels, out_channels, int_emb_size, basis_emb_size_dist, basis_emb_size_angle,
        basis_emb_size_torsion, out_emb_channels, num_spherical, num_radial, envelope_exponent, num_before_skip,
        num_after_skip, num_output_layers, dropout_rate, use_node_features, act=swish, output_init='GlorotOrthogonal'):
        super(SphereNet, self).__init__()

        self.cutoff = cutoff
        self.dropout_rate = dropout_rate
        self.init_e = init(num_radial, hidden_channels, dropout_rate, act, use_node_features=use_node_features)
        self.init_v = update_v(hidden_channels, out_emb_channels, out_channels, num_output_layers, dropout_rate, act, output_init)
        self.init_u = update_u()
        self.emb = emb(num_spherical, num_radial, self.cutoff, envelope_exponent)

        self.update_vs = torch.nn.ModuleList([
            update_v(hidden_channels, out_emb_channels, out_channels, num_output_layers, dropout_rate, act, output_init) for _ in range(num_layers)])

        self.update_es = torch.nn.ModuleList([
            update_e(hidden_channels, int_emb_size, basis_emb_size_dist, basis_emb_size_angle, basis_emb_size_torsion, num_spherical, num_radial, num_before_skip, num_after_skip, dropout_rate, act) for _ in range(num_layers)])
    ...
    def forward(self, batch_data):
        ...
        dist, angle, torsion = dist, angle, torsion
